...
class DataLoader(object):
    ...
    def load_train_batch(self):
        ...
        tgt_image, src_image, tgt_right_image, src_right_image = \
        ...
        tgt_image, src_image, tgt_right_image, src_right_image, intrinsics = \
            tf.train.shuffle_batch([tgt_image, src_image, tgt_right_image, src_right_image, intrinsics],
                                   opt.batch_size, capacity, min_after_dequeue, opt.num_threads, seed)


        # Data augmentation
        image_all = tf.concat([tgt_image, src_image, tgt_right_image, src_right_image], axis=3)

        image_all, intrinsics = self.data_augmentation(
            image_all, intrinsics, opt.img_height, opt.img_width)
        tgt_image = image_all[:, :, :, :3]
        src_image = image_all[:, :, :, 3:6]
        tgt_right_image = image_all[:, :, :, 6:9]
        src_right_image = image_all[:, :, :, 9:]

        intrinsics = self.get_multi_scale_intrinsics(intrinsics, opt.num_scales)


        return tgt_image, src_image, intrinsics, tgt_right_image, src_right_image
    ...
    def unpack_image_sequence(self, image_seq, img_height, img_width, num_source):
        ...
        tgt_start_idx = int(img_width)    # if 2 frames , num_source is 1
        rig_start_idx = int(img_width * (num_source+1))  # 2 * img_width
        src_start_idx = int(img_width * (num_source+2))  # 3 * img_width

        tgt_image = tf.slice(image_seq,
                             [0, 0, 0],
                             [-1, img_width, -1])
        src_image = tf.slice(image_seq,
                               [0, int(tgt_start_idx), 0],
                               [-1, int(img_width ), -1])
        ...
        # Right frames
        tgt_right_image = tf.slice(image_seq,
                             [0,rig_start_idx, 0],
                             [-1, img_width, -1])

        src_right_image = tf.slice(image_seq,
                             [0, src_start_idx, 0],
                             [-1, img_width, -1])

        tgt_right_image.set_shape([img_height, img_width, 3])
        src_right_image.set_shape([img_height, img_width, 3])


        return tgt_image, src_image, tgt_right_image, src_right_image
