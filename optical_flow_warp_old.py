...
def transformer_old(U, flo, out_size, name='SpatialTransformer', **kwargs):
    ...
    def _repeat(x, n_repeats):
        with tf.variable_scope('_repeat'):
            rep = tf.transpose(
                tf.expand_dims(
                    tf.ones(shape=tf.stack([int(n_repeats), ])), 1), [1, 0])
            rep = tf.cast(rep, 'int32')  # 类型转换
    ...
    def _meshgrid(height, width):
        with tf.variable_scope('_meshgrid'):
            ...
            #############################
            # height = 8.0 , width = 26.0

            x_t = tf.matmul(
                tf.ones(shape=tf.stack([int(height), 1])),    # height = 8.0
                tf.transpose(
                    tf.expand_dims(tf.linspace(-1.0, 1, int(width)), 1), [1, 0]))
            y_t = tf.matmul(
                tf.expand_dims(tf.linspace(-1.0, 1.0, int(height)), 1),
                tf.ones(shape=tf.stack([1, int(width)])))
