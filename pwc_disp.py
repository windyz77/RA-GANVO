...
def feature_pyramid_disp(image, reuse):
    with tf.variable_scope('train_feature_net_flow'):
        with slim.arg_scope(
            [slim.conv2d, slim.conv2d_transpose],
                weights_regularizer=slim.l2_regularizer(0.0004),
                activation_fn=leaky_relu,
                variables_collections=["train_flownet"],
                reuse=reuse):
...
def construct_model_pwc_full_disp(feature1, feature2, image1, neg=False):
    ...
    flow6, _ = optical_flow_decoder_dc(cv6, level=6)  # (4, 4, 13, 2)
    if neg:
    ...
    flow3to2 = tf.image.resize_bilinear(flow3,
                                        [int(H / (2**2)), int((W / (2**2)))]) * 2.0  # why yao *2.0
...
def pwc_disp(tgt_image, tgt_image_r, tgt_feature, tgt_feature_r, reuse):
    min_disp = 1e-6
    with tf.variable_scope('train_tgt_disp', reuse=reuse):
            tgt_disp = construct_model_pwc_full_disp(tgt_feature, tgt_feature_r, tgt_image, neg=True)

    with tf.variable_scope('train_tgt_disp_r', reuse=reuse):
            tgt_disp_r = construct_model_pwc_full_disp(tgt_feature_r, tgt_feature, tgt_image_r, neg=False)

    return [tf.concat([tgt + min_disp, tgt_r + min_disp], axis=0) for tgt, tgt_r in zip(tgt_disp, tgt_disp_r)]
