...
def cost_volumn(feature1, feature2, d=4):
    batch_size, H, W, feature_num = map(int, feature1.get_shape()[0:4])  # 4 , 4, 13, 192

    feature2 = tf.pad(feature2, [[0, 0], [d, d], [d, d], [0, 0]], "CONSTANT")   # (4, 12,21,192)
    cv = []
    ...
    return tf.concat(cv, axis=3)   # shape=(4, 4, 13, 81)
...
def optical_flow_decoder_dc(inputs, level):
    with slim.arg_scope(
        [slim.conv2d, slim.conv2d_transpose],
            ...
            activation_fn=leaky_relu):
        cnv1 = slim.conv2d(
            inputs, 128, [3, 3], stride=1, scope="cnv1_fd_" + str(level))  # shape=(4, 4, 13, 128)
        cnv2 = slim.conv2d(
        ...
        cnv5 = slim.conv2d(
            ...
            32, [3, 3],
            stride=1,
            scope="cnv5_fd_" + str(level))  # shape=(4, 4, 13, 32)
        flow = slim.conv2d(
            tf.concat(
                [cnv4, cnv5], axis=3),
            2, [3, 3],
...
def construct_model_pwc_full(image1, image2, feature1, feature2, reuse):
    with tf.variable_scope('train_flow_pwcnet', reuse=reuse):
