...
class kitti_odom_loader(object):
    ...
    def is_valid_sample(self, frames, tgt_idx):
        ...
        half_offset = int((self.seq_length - 1))
    ...
    def load_image_sequence(self, frames, tgt_idx, seq_length):
        ...
        # add tgt right picture
        curr_idx = tgt_idx
        curr_drive, curr_frame_id = frames[curr_idx].split(' ')
        curr_img = self.load_image_r(curr_drive, curr_frame_id)
        curr_img = scipy.misc.imresize(curr_img, (self.img_height, self.img_width))
        image_seq.append(curr_img)

        # add src right picture (next frame of tgt )
        curr_idx = tgt_idx + 1
        curr_drive, curr_frame_id = frames[curr_idx].split(' ')
        curr_img = self.load_image_r(curr_drive, curr_frame_id)
        curr_img = scipy.misc.imresize(curr_img, (self.img_height, self.img_width))
    ...
    def load_image_r(self, drive, frame_id):
        img_file = os.path.join(self.dataset_dir, 'sequences', '%s/image_3/%s.png' % (drive, frame_id))
        img = scipy.misc.imread(img_file)
        return img
