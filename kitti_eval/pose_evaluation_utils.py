...
def quaternion_to_rotation_matrix(quat):
    ...
    rot_matrix = np.array(
        [[(1.- 2*y*y- 2*z*z), -(2*x*y + 2*w*z),-( 2*x*z - 2*w*y), quat[0]],
         [-(2*x*y - 2*w*z), (1.-2*x*x-2*z*z), -(2*y*z+2*w*x), quat[1]],
         [-(2*x*z+2*w*y), -(2*y*z-2*w*x), (1.-2*x*x-2*y*y), quat[2]],
         [0., 0., 0., 1.0]],
        dtype=float)
    return rot_matrix
...
def compute_ate(gtruth_file, pred_file):
    ...
    gtruth = np.array([[float(value) for value in gtruth_list[a][:]] for a, b in matches])
    pred = np.array([[float(value) for value in pred_list[b][:]] for a, b in matches])
    gt_matrix = []
    pred_matrix = []
    rotation = []
    quarter_gt = []
    ...
    for i in range(len(gtruth)):
        ...
        w = gtruth[i][6:]
        r = gtruth[i][3:6]
        temp_pose_gt = np.concatenate([t, w, r], axis=0)
        quarter_gt.append(temp_pose_gt)
        temp_matrix = quaternion_to_rotation_matrix(temp_pose_gt)
        gt_matrix.append(temp_matrix[:3, :3])
    for i in range(len(pred)):
        t = pred[i][0:3]
        w = pred[i][6:]
        r = pred[i][3:6]
        temp_pose_pred = np.concatenate([t, w, r], axis=0)
        quarter_pred.append(temp_pose_pred)
        temp_matrix = quaternion_to_rotation_matrix(temp_pose_pred)
        pred_matrix.append(temp_matrix[:3, :3])
    pose_error_quarter = []
    for i in range(len(gt_matrix)):
        pose_error = np.dot(gt_matrix[i], np.linalg.inv(pred_matrix[i]))
        pose_error_quarter.append((quarter_gt[i]-quarter_pred[i]))
        a = pose_error[0, 0]
        b = pose_error[1, 1]
        c = pose_error[2, 2]
        d = 0.5 * (a + b + c - 1.0)
        theta = np.arccos(max(min(d, 1.0), -1.0))
        theta = theta / np.pi *180
        rotation.append(theta)
    roer = np.sqrt(np.sum(np.array(rotation) ** 2)) / len(matches)
    return roer, rmse
